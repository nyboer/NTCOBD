...
class MyStuff ( Screen ):
    ...
    def draw( self, width, height ):
        ...
        div = 2
        matrix = cairo.Matrix ( 1, 0, 0, 1, width/div, height/div )
        #matrix = cairo.Matrix ()
        ...
        ## We restore to a clean context, so drawings after this restore are not transformed
        cr.restore ( )

        ## Let's draw a crosshair so we can identify 0,0
        ## Drawn last to be above the red square.
        self.drawcross ( cr )
        self.speed += 0.1 # Change to 0 to see if rotation is working...
        if self.speed > 1: self.speed=0
        self.dial ( cr, width, height, self.speed)
    ...
    def dial(self, ctx, width, height, val=0):
        div = 10
        # xc = width/div
        # yc = height/div
        xc = 0
        ...
        # needle
        ctx.arc(xc, yc, radius, needle, needle)
        ctx.line_to(xc, yc)
        ctx.stroke()
